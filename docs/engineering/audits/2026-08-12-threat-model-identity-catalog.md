...
## Addendum — PCA-20260812-001 resolvido (2026-08-19)

O risco residual de maior severidade deste documento (§3.6, "ausência de `required_status_checks`") foi corrigido: `main` agora exige, via `gh api` (confirmado ao vivo), os 7 checks do Tier A como `required_status_checks.contexts` (`strict: true`) — `Verify (typecheck, lint, format, unit)`, `Integration (fast, DynamoDB Local)`, `Dependency Review`, `npm audit (high/critical)`, os dois checks de `Security Scans (Semgrep + Gitleaks)`, e `Validate Infra (Terraform + TFLint + Trivy)`. Decisão tomada por Marcelo (ver `full-project-audit.md` — item já registrado como exigindo decisão humana), aplicada durante a primeira revisão conjunta Claude/Codex de arquitetura (`AGENTS.md` §2). Corrigido junto: `security.yml` rodava Semgrep/Gitleaks duas vezes por PR (trigger `pull_request` próprio além da chamada via `ci.yml`) — removido o trigger duplicado, o que também simplifica a lista de contexts acima (sem os nomes "soltos" `SAST (Semgrep)`/`Secret Detection (Gitleaks)` que existiam antes).
